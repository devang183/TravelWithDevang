function hashString(str) {
  ...
  const geojson = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'placeofworship.geojson'))
  );  
  const cityMapData = {};
  ...
  geojson.features.forEach(feature => {
    ...
    // Write output to DublinCityPlaceOfWorshipData.js
    const outputPath = path.join(__dirname, 'DublinCityPlaceOfWorshipData.js');
    const outputCode = `export const DublinCityPlaceofWorshipData = ${JSON.stringify(cityMapData, null, 2)};`;

    fs.writeFileSync(outputPath, outputCode);
    console.log('✅ Dublin Places of Worship data generated successfully.');