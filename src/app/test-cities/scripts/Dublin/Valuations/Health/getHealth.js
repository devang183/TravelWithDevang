...
// --- CONFIG ---
const inputFile = path.join(__dirname, 'health.json'); // your GeoJSON file
const outputFile = path.join(__dirname, 'dublin-properties-Health.json');
...
const formatted = geojsonData.features
  .filter(f => f.properties?.category === 'health')
  .map(f => {
    ...
    const name = `${props.name || 'Health'} ${hashString(props.propertynumber || '')}`;
...
console.log(`✅ Health JSON with lat/lng and hashId saved to ${outputFile}`);