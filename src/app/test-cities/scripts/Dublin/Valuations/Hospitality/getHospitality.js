...
// --- CONFIG ---
const inputFile = path.join(__dirname, 'hospitality.json'); // your GeoJSON file
const outputFile = path.join(__dirname, 'dublin-properties-Hospitality.json');
...
const formatted = geojsonData.features
  .filter(f => f.properties?.category === 'hospitality')
  .map(f => {
    ...
    const name = `${props.name || 'Hospitality'} ${hashString(props.propertynumber || '')}`;
...
console.log(`✅ Hospitality JSON with lat/lng and hashId saved to ${outputFile}`);