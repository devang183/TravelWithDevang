...
// --- CONFIG ---
const inputFile = path.join(__dirname, 'leisure.json'); // your GeoJSON file
const outputFile = path.join(__dirname, 'dublin-properties-leisure.json');
...
const formatted = geojsonData.features
  .filter(f => f.properties?.category === 'LEISURE') // only LEISURE
  .map(f => {
    ...
    const name = `${props.name || 'Leisure'} ${hashString(props.propertynumber || '')}`;
...
console.log(`✅ Leisure JSON with lat/lng saved to ${outputFile}`);