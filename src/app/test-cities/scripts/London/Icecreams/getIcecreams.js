...
const { default: build } = require('next/dist/build');
  ...
  geojson.features.forEach(feature => {
    if (
        ...
        feature.properties.amenity === 'ice_cream'
      )  {
      const coords = [feature.geometry.coordinates[1], feature.geometry.coordinates[0]];

      const name = `${feature.properties.name || 'Ice Cream'} ${hashString(feature.properties['@id'] || '')}`; 
...
const description = details.length > 0
    ...
    : "No additional information available";
      ...
      cityMapData['london-2024'].push({
        ...
        url,
        category:'icecream',
        keywords: [street, suburb, ccity, postcode, phone, url, curl, email]
    ...
    // Write output to LondonCityIceCreamData.js
    const outputPath = path.join(__dirname, 'LondonCityIceCreamData.js');
    const outputCode = `export const LondonCityIceCreamData = ${JSON.stringify(cityMapData, null, 2)};`;

    fs.writeFileSync(outputPath, outputCode);
    console.log('✅ London IceCream data generated successfully.');