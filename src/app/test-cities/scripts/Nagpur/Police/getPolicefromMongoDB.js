...
const outputFile = path.join(__dirname, "nagpur-police-pins.json"); // output file
...
async function generateCityMapPins() {
  ...
  try {
    ...
    const policeStations = await collection.find({}).toArray();

    const formattedPins = policeStations.map((s) => {
      const name = `${s.name} Police ${hashString(s.name)}`;
      ...
      return {
        ...
        name,
        description,
        category: "police", // set category for police stations
        keywords,
      };
    });
