...
async function fetchAndInsert() {
  try {
    ...
    const parks = response.data.elements
      .filter((e) => e.tags && e.tags.leisure === "park")
      .map((e) => ({
        name: e.tags.name || "Park",
    ...
    const result = await collection.insertMany(parks);
    console.log(`✅ Inserted ${result.insertedCount} parks into MongoDB`);
