...
const CityAttractionsGalway = () => {
  ...
  const cityName = 'Galway'; // Hardcoded for Galway

  useEffect(() => {
    const fetchAttractions = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/attractions?city=Galway&limit=200`);
    ...
    fetchAttractions(); // Always fetch for Galway
...
export default CityAttractionsGalway;
