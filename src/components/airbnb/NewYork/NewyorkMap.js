...
import { neighborhoodsList } from "@/components/airbnb/NewYork/neighbourhoodList.js";
...
export default function NewYorkMap() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      setLoading(true);
      try {
        const res = await fetch("/api/listingsNewYork");
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-xl">Loading New York listings...</div>
      </div>
    );
  }
